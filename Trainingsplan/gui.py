...
class TrainingAppGUI:
    ...
    def refresh_all(self):
        ...
        if goal_min > 0:
            status = "✅" if w["minuten"] >= goal_min else "❌"
            msg.append(f"{status} Minuten-Ziel ({w['minuten']}/{goal_min})")
        #Übungen-Ziel prüfen
        goal_ex = u["ziele"].get("woechentlich_uebungen", 0)
        if goal_ex > 0:
            status = "✅" if w["uebungen"] >= goal_ex else "❌"
            msg.append(f"{status} Übungen-Ziel ({w['uebungen']}/{goal_ex})")
